...
CREATE POLICY "Users can create own listings" ON market_listings FOR INSERT TO authenticated WITH CHECK (auth.uid() = seller_id);

DROP POLICY IF EXISTS "Users can update own listings" ON market_listings;
CREATE POLICY "Users can update own listings" ON market_listings FOR UPDATE TO authenticated USING (auth.uid() = seller_id) WITH CHECK (auth.uid() = seller_id);

DROP POLICY IF EXISTS "Users can delete own listings" ON market_listings;
CREATE POLICY "Users can delete own listings" ON market_listings FOR DELETE TO authenticated USING (auth.uid() = seller_id);
